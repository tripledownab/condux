...
public interface IModelClient
{
    ...
    /// <summary>
    /// Whether this provider can drive the agentic tool loop. False falls back to the single-shot patch
    /// proposal, which is the point of keeping that path alive: tool-calling quality varies sharply, and a
    /// model that fumbles it loops uselessly while spending the customer's tokens. A capability rather
    /// than a setting, so the choice follows from the resolved provider instead of becoming one more thing
    /// a customer has to understand.
    /// </summary>
