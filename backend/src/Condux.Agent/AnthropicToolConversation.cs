    ...
    HttpClient http, string apiKey, string model, string systemPrompt, string firstMessage) : IAgentConversation
{
        ...
        IReadOnlyList<AgentToolResult> results, CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            throw new InvalidOperationException(
                $"Anthropic returned {(int)response.StatusCode}: {ModelApiError.Describe(payload)}");
        }
    ...
    /// <summary>
    /// The tools offered to the model. Set from the workspace's capabilities, so a run without a sandbox
    /// is never told about a command tool it cannot honour.
    /// </summary>
