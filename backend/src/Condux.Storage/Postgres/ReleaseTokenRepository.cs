...
/// <summary>A scoped release token. Only <c>TokenHash</c> is stored; the raw token is shown once.</summary>
public sealed record ReleaseToken(
    Guid Id, long ProjectId, string Name,
    DateTimeOffset CreatedAt, DateTimeOffset? LastUsedAt, DateTimeOffset? RevokedAt);

/// <summary>Mints, lists, resolves, and revokes scoped release tokens (CI records releases with them).</summary>
public sealed class ReleaseTokenRepository(string connectionString)
{
    private const string InsertSql = """
        INSERT INTO release_tokens (id, project_id, token_hash, name)
    ...
    private const string ListSql = """
        ...
        FROM release_tokens WHERE project_id = @project ORDER BY created_at DESC;
        """;

    // Resolve a presented token to its project only if live (not revoked); stamp last_used_at so the
    // dashboard can show when CI last used it. One statement, so the check + stamp are atomic.
    private const string ResolveSql = """
        UPDATE release_tokens SET last_used_at = now()
        WHERE token_hash = @hash AND revoked_at IS NULL
        RETURNING project_id;
        """;

    private const string RevokeSql =
        "UPDATE release_tokens SET revoked_at = now() WHERE project_id = @project AND id = @id AND revoked_at IS NULL;";

    public async Task<ReleaseToken> CreateAsync(
    ...
    public async Task<IReadOnlyList<ReleaseToken>> ListByProjectAsync(long projectId, CancellationToken ct = default)
    {
        await using var conn = new NpgsqlConnection(connectionString);
        await conn.OpenAsync(ct);
        await using var cmd = new NpgsqlCommand(ListSql, conn);
        cmd.Parameters.AddWithValue("project", projectId);
        var list = new List<ReleaseToken>();
        await using var reader = await cmd.ExecuteReaderAsync(ct);
    ...
    private static ReleaseToken Read(NpgsqlDataReader r) => new(
