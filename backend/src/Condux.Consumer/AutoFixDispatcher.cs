...
/// assembly + quota reservation + publish the manual RequestFix does, minus the human click. The run
/// still ends in a human-reviewed <b>draft</b> PR (auto means auto-<i>request</i>, never auto-merge).
...
public sealed class AutoFixDispatcher(
    ...
    IFixRequestPublisher publisher, ConductorPauseNotifier pauseNotifier,
    ILogger<AutoFixDispatcher> logger)
{
        ...
        long projectId, UpsertResult upsert, Event sampleEvent, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var job = new FixJob(upsert.Id, repo.RepoFullName, repo.DefaultBranch, AutoActor, DefaultModel)
            {
